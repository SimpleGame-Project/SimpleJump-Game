...
{
    public Transform platformsParent; // 발판 부모 오브젝트 (Platforms)
    ...
    void Start()
    {
        ...
        if (mainCamera == null)
        {
            mainCamera = Camera.main; // 씬의 메인 카메라 자동 할당
        }
    }

    void Update()
    {
        CheckPlatformsOutOfCamera(); // 매 프레임마다 카메라 밖 발판 체크
    }
